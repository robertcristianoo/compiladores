...
Exp* Arvore_parse::simplificado(No_arv_parse * no) {
  ...
  case 2: case 10: case 16:
    return new ExpDoisOp(no->filhos[1]->regra, OperadorBin::gera_operador_bin(no->filhos[1]->regra), simplificado(no->filhos[0]), simplificado(no->filhos[2]));
  case 26: case 27: case 28: 
    return new ExpUmOp(no->regra, OperadorUnario::gera_operador_unario(no->regra), simplificado(no->filhos[1]));
  default:
    return NULL;
  }
}
